...
function duplicateString(str, int) {
  const newArray = [];
...
function duplicateString2(str, int) {
  return Array(int).fill(str);
}
...
//     }
//   }
// } // ends up only looping once if first values are equal
...
function compareArr(arr1, arr2) {
  ...
  arr1.sort(); // sorts each arr in alphabetical order
  arr2.sort();

  for (let i = 0; i < arr1.length; i++) {
    // if each index is equal, true. if any are != exits and returns false
    if (arr1[i] !== arr2[i]) {
      return false;
    }
  }
